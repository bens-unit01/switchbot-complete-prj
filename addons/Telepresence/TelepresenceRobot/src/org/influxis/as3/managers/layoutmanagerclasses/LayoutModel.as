/**
 * LayoutModel - Copyright © 2010 Influxis All rights reserved.
**/

package org.influxis.as3.managers.layoutmanagerclasses 
{
	//Flash Classes
	...
	public class LayoutModel extends DataProviderOld implements ISimpleSprite
	{
		...
		public function LayoutModel( ...children ): void 
		{
			super();
		...
		private function __addChild( index:int, display:ISimpleSprite, adjustHook:Boolean = false ): Boolean
		{
			...
			if ( _data.length > 0 ) display.hook( _data[_data.length - 1] as ISimpleSprite, this, (direction==RotateState.VERTICAL?SizeUtils.BOTTOM:SizeUtils.RIGHT), (direction==RotateState.VERTICAL?SizeUtils.CENTER:SizeUtils.MIDDLE), false );  
		...
		private function __removeChild( index:int, display:ISimpleSprite, adjustHook:Boolean = false ): Boolean
		{
			...
			if ( adjustHook && display ) display.hook(  _data[_data.length - 1] as ISimpleSprite, null, (direction == RotateState.VERTICAL?SizeUtils.BOTTOM:SizeUtils.RIGHT), (direction == RotateState.VERTICAL?SizeUtils.CENTER:SizeUtils.MIDDLE), false );
			if ( !lock ) arrange();
		...
		private function __changeSizeListeners(): void
		{
			...
			for each ( var o:Object in _data )
			{
				...
				display.removeEventListener( _sDirection == RotateState.VERTICAL ? SimpleEventConst.RESIZE_WIDTH : SimpleEventConst.RESIZE_HEIGHT, handleResize );
				display.addEventListener( _sDirection == RotateState.HORIZONTAL ? SimpleEventConst.RESIZE_WIDTH : SimpleEventConst.RESIZE_HEIGHT, handleResize );
				display.updateHook( (direction == RotateState.VERTICAL?SizeUtils.BOTTOM:SizeUtils.RIGHT), (direction == RotateState.VERTICAL?SizeUtils.CENTER:SizeUtils.MIDDLE) );
			}
		}
		...
		{
			SizeUtils.hookTarget( this, _hookTarget, _hookPosition, _hookPadding, _hookInner );
			//trace( "__onHookChange: " + id, this.height, _hookTarget.height, _hookContainer.height, _hookAlign, _hookPosition, event );
			if ( _hookPosition == SizeUtils.TOP || _hookPosition == SizeUtils.BOTTOM )
			{
				SizeUtils.moveByTargetX( this, _hookContainer, _hookAlign, _hookPadding );
			}else if( _hookAlign ){
				SizeUtils.moveByTargetY( this, _hookContainer, _hookAlign, _hookPadding );
			}
		}
		...
		**/
		...
		protected final function checkDimensions(): void
		{
			...
			if ( _width != w || _height != h )
			{
				...
				{
					_width = w;
					dispatchEvent( new Event(SimpleEventConst.RESIZE_WIDTH) );
				}
				
				if ( _height != h ) 
				{
					_height = h;
					dispatchEvent( new Event(SimpleEventConst.RESIZE_HEIGHT) );
				}
				onSizeChanged();
			}
		}
		...
		protected final function checkPositions(): void
		{ 
			...
			if ( this.x != x || this.y != y )
			{
				...
				{
					_x = x;
					dispatchEvent( new Event(SimpleEventConst.MOVE_X) );
				}
				
				if ( _y != y )
				{
					_y = y;
					dispatchEvent( new Event(SimpleEventConst.MOVE_Y) );
				}
				onPositionChanged();
			}
		...
		{
			arrange();
			dispatchEvent(new Event(SimpleEventConst.RESIZE));
		}
		...
		protected function onPositionChanged(): void
		{
			...
			dispatchEvent(new Event(SimpleEventConst.MOVE));
		}
			...
			for each( var i:Object in _data )
			{
				...
				if ( display )
				{
					if ( !isNaN(display[percentProp]) )
					{
						...
						totalPixels =+ display[sizeProp];
					}
					//trace( "arrange(); " + display[percentOther], this[sizeOther] );
					if ( !isNaN(display[percentOther]) ) display[sizeOther] = this[sizeOther];
				}
			}
			
			//trace( "arrange()2; " + percentOther, sizeOther, height, percentHeight );
			//trace( "    " );
			totalPixels = this[sizeProp] - totalPixels;
