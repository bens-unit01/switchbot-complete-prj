...
package org.influxis.as3.codecs
{
	//Flash Classes
	...
	public final class JPEGAsyncEncoder extends EventDispatcher
	{
		...
		public function JPEGAsyncEncoder(quality:int=50)
		{
			if (quality <= 0)
				quality = 1;
		...
		{ 
			return __internalEncode(raw, width, height); 
		}*/
		
		public function encode( image:BitmapData, run:Boolean = true ): Boolean
		{ 
			return __internalEncode(image, image.width, image.height, run);
		}
		
		public function stop(): void
		{
			if ( _stopped || isNaN(_timerInt) || !Working ) return;
			
			clearTimeout(_timerInt);
			_timerInt = NaN;
		}
		...
		{
			if ( !_stopped || !isNaN(_timerInt) || !Working ) return;
			_timerInt = setTimeout(__asyncLoop, WaitTime, _lastXPos, _lastYPos);
		}
		...
		private function __internalEncode( newSource:BitmapData, width:int, height:int, run:Boolean ): Boolean
		{
			...
			setTimeout(__startEncode, 10, run );
			return true;
		}
		
		private function __startEncode( run:Boolean ): void
		{
			...
			_lastXPos = 0; _lastYPos = 0;
			if( run ) _timerInt = setTimeout(__asyncLoop, WaitTime, _lastXPos, _lastYPos);
		} 
		
		private function __asyncLoop(xpos:int, ypos:int):void
		{			
			...
			_lastXPos = xpos; _lastYPos = ypos;
			_timerInt = setTimeout(__asyncLoop, WaitTime, xpos, ypos);
		}
		
		private function __finishEncode():void
		{
			//EOI
			if (bytepos >= 0)
			{
		...
		public function set pixelsPerIteration( value:int ): void
		{ 
			PixelsPerIter = value; 
		}
		...
		{ 
			return ObjectUtils.cloneObject(byteout) as ByteArray; 
		}
		
		public function get bytesLoaded(): int
		{
			return CurrentTotalPos;
		}
