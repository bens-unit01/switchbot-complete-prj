...
#include "nst_ble_bls.h"
#include "nst_ble_ahs.h"
#include "nst_ble_rrs.h"
#include "nst_ble_mps.h"
#include "nst_ble_dis.h"
#include "nst_ble_pios.h"
...
#define WAKEUP_BUTTON_PIN               16                                          /**< Button used to wake up the application. */
// YOUR_JOB: Define any other buttons to be used by the applications:
// #define MY_BUTTON_PIN                   16

#define ADV_LED_PIN_NO                  26                                          /**< Is on when device is advertising. */
#define CON_LED_PIN_NO                  27                                          /**< Is on when device has connected. */
#define LED1                            0
#define LED2                            1

#define DEVICE_NAME                     "WowWee"                                    /**< Name of device. Will be included in the advertising data. */
...
{
    nrf_gpio_pin_set(ADV_LED_PIN_NO);
    nrf_gpio_pin_set(CON_LED_PIN_NO);
...
{
    nrf_gpio_cfg_output(ADV_LED_PIN_NO);
    nrf_gpio_cfg_output(CON_LED_PIN_NO);
...
static void advertising_init(void)
{
    ...
    uint8_t                  m_addl_adv_manuf_data[16];

    // YOUR_JOB: Use UUIDs for service(s) used in your application.
    ble_uuid_t adv_uuids[] = {{0xFFF0, BLE_UUID_TYPE_BLE}, {0xFFB0, BLE_UUID_TYPE_BLE}};

    // Build and set advertising data
    memset(&advdata, 0, sizeof(advdata));
    memset(m_addl_adv_manuf_data, 0, sizeof(m_addl_adv_manuf_data));
    manuf_data.company_identifier = ((PRODUCT_ID & 0xFF) << 8) + ((PRODUCT_ID & 0xFF00) >> 8);;
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
            nrf_gpio_pin_set(CON_LED_PIN_NO);
            nrf_gpio_pin_clear(ADV_LED_PIN_NO);
        ...
        case BLE_GAP_EVT_DISCONNECTED:
            nrf_gpio_pin_clear(CON_LED_PIN_NO);
...
static void ble_evt_dispatch(ble_evt_t * p_ble_evt)
{
    ...
    */
    ...
    ble_bls_on_ble_evt(p_ble_evt);
    ble_dis_on_ble_evt(p_ble_evt);
    ble_dts_on_ble_evt(p_ble_evt);
    ble_ahs_on_ble_evt(p_ble_evt);
    ble_rrs_on_ble_evt(p_ble_evt);
    ble_mps_on_ble_evt(p_ble_evt);
    ble_pios_on_ble_evt(p_ble_evt);
}
...
//    err_code = sd_ble_priv_radio_active_cpu_mode_set(BLE_PRIV_RADIO_ACTIVE_CPU_MODE_AWAKE);
//    APP_ERROR_CHECK(err_code);
...
static void buttons_init(void)
{
    ...
    {
        {WAKEUP_BUTTON_PIN, APP_BUTTON_ACTIVE_LOW, NRF_GPIO_PIN_PULLUP, NULL},
