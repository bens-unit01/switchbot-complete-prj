...
extern uint8_t ble_mps_connected_broadcast_data_char_buffer[20];
...
static uint8_t * m_get_adv_packet(void)
{
    ...
    adv_pdu[length + data_length++] = 0x00;
    adv_pdu[length + data_length++] = 0x0F;

    memcpy(&adv_pdu[length + data_length], ble_mps_connected_broadcast_data_char_buffer, sizeof(ble_mps_connected_broadcast_data_char_buffer));

    adv_pdu[ADV_PACK_LENGTH_IDX]   = 36;
    adv_pdu[ADV_DATA_LENGTH_IDX]   = 23;
